...
export default function SignInPage() {
  ...
  useEffect(() => {
    fetch(withBasePath('/api/auth/providers'))
      ...
      .catch(() => {});
    // Fetch CSRF token for form-based sign-in
        ...
        <CardContent className="space-y-4">
          ...
          {githubEnabled && (tab === 'login' || tab === 'register') && (
            <>
              ...
              <form method="POST" action={withBasePath('/api/auth/signin/github')}>
                <input type="hidden" name="csrfToken" value={csrfToken} />
                <input type="hidden" name="callbackUrl" value={typeof window !== 'undefined' ? window.location.origin + '/furinakit/' : ''} />
                <Button
                  type="submit"
                  variant="outline"
                  className="w-full"
