...
**一站式文件处理平台 · 87 款专业工具 · Web / API / CLI 三端协同**
...
[![Tests](https://img.shields.io/badge/Tests-472%20passed-brightgreen?style=flat-square)](#测试覆盖)
...
FurinaKit 是一个现代化的在线工具箱，提供 **87 款专业文件处理工具**，涵盖 PDF、图片、视频、音频、文本、开发辅助等多个领域。采用 Next.js 16 构建，支持 Web 界面、REST API 和命令行工具三种使用方式，满足不同场景的需求。

### 🎯 核心特性

| 特性 | 说明 |
|------|------|
| 🛠️ **87 款工具** | PDF 处理、图片编辑、视频转换、音频剪辑、文本工具、开发辅助等 |
| 🌍 **四语言支持** | 中文 / English / 日本語 / 한국어 完整国际化 |
| 🎨 **精美界面** | 基于 shadcn/ui 的现代化设计，支持亮色/暗色主题 |
| 📱 **响应式设计** | 完美适配桌面端、平板和移动端 |
| ⚡ **三端可用** | Web 界面 / REST API / 命令行工具 |
| 🧪 **质量保障** | 472 个单元测试 + E2E 测试，100% 工具覆盖 |
...
<details>
<summary><strong>🖼️ 图片处理 (13 个)</strong></summary>
...
| image-add-text | 添加文字叠加层 |
| image-merge | 拼接多张图片 |
| image-to-ico | 生成 ICO 图标文件 |
| image-exif | 查看/编辑 EXIF 信息 |
| image-compare | 图片差异对比 |
| gif-maker | GIF 动图制作 |
| svg-optimize | SVG 优化压缩 |

</details>

<details>
<summary><strong>📝 文本处理 (15 个)</strong></summary>
...
<details>
<summary><strong>🎬 视频处理 (4 个)</strong></summary>
...
<details>
<summary><strong>💻 开发工具 (27 个)</strong></summary>
...
| color-palette | 调色板生成器 |
| css-gradient | CSS 渐变生成器 |
...
| ascii-art | ASCII 艺术字生成 |
| code-minify | 代码压缩混淆 |
| dns-lookup | DNS 查询工具 |
| ssl-checker | SSL 证书检查 |
| ip-lookup | IP 地址查询 |
| user-agent-parser | UA 字符串解析 |
| json-schema-validate | JSON Schema 校验 |
| openapi-viewer | OpenAPI 文档查看 |
| font-preview | 字体预览工具 |
...
<details>
<summary><strong>🎨 手工创意 (3 个)</strong></summary>

| 工具 | 说明 |
|------|------|
| perler-beads | 拼豆图纸生成器，支持多种色板、智能取色、线稿引导、颜色统计、编辑模式 |
| business-card | 电子名片生成器 |
| pomodoro | 番茄钟计时器 |
| word-cloud | 词云生成器 |
...
FurinaKit is a modern online toolbox providing **87 professional file processing tools** across PDF, image, video, audio, text, and developer utilities. Built with Next.js 16, it supports Web UI, REST API, and CLI interfaces to meet diverse workflow needs.

### 🎯 Key Features

| Feature | Description |
|---------|-------------|
| 🛠️ **87 Tools** | PDF processing, image editing, video conversion, audio trimming, text utilities, dev tools |
...
| 🧪 **Quality** | 472 unit tests + E2E tests with 100% tool coverage |
...
FurinaKit は、PDF、画像、動画、音声、テキスト、開発ツールなど **87 のプロフェッショナルなファイル処理ツール** を提供するモダンなオンラインツールボックスです。Next.js 16 で構築され、Web UI、REST API、CLI の 3 つのインターフェースをサポートしています。

### 🎯 主な特徴

| 特徴 | 説明 |
|------|------|
| 🛠️ **87 ツール** | PDF 処理、画像編集、動画変換、音声トリミング、テキストツール、開発ツール |
...
| 🧪 **品質保証** | 472 の単体テスト + E2E テスト、100% カバレッジ |
...
FurinaKit은 PDF, 이미지, 비디오, 오디오, 텍스트, 개발 도구 등 **87개의 전문 파일 처리 도구**를 제공하는 현대적인 온라인 도구 상자입니다. Next.js 16으로 구축되었으며, Web UI, REST API, CLI의 3가지 인터페이스를 지원합니다.

### 🎯 주요 기능

| 기능 | 설명 |
|------|------|
| 🛠️ **87개 도구** | PDF 처리, 이미지 편집, 비디오 변환, 오디오 트리밍, 텍스트 도구, 개발 도구 |
...
| 🧪 **품질 보증** | 472개 단위 테스트 + E2E 테스트, 100% 커버리지 |
